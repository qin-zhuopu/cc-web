...
C2 是 CodePilot Web 里**持有"一次 AI 流式回合（StreamSession）的实时生命周期与多 Runtime 抽象"**的限界上下文。它负责：接受一次发送请求 → 选中某个 Runtime（ClaudeCode SDK / Native HTTP / Codex app-server）→ 把该 Runtime 的原生事件流归一化成统一的 `AgentStreamEvent` → 维护这次回合的实时相位状态机（`active → settling → terminal`）→ 支持中断（abort/interrupt）。它**自己不持久化会话与消息**（回合产物经 C1 用例落库），也**不做子 agent 编排**（那是 C3，复用 C2 的 `AgentRuntimePort`）。

## 2. 解决什么问题

C2 对应的是现有 Electron 版 CodePilot **bug 最密集**的模块——`stream-session-manager.ts`、`claude-client.ts`、`conversation-registry.ts`、`codex/app-server-manager.ts`。这批文件的核心痛点是"实时流式状态"被散落成一堆可变字段与定时器，语义没有被固化成领域不变量，导致同一类中断 / 卡死问题反复出现：
...
- **多 Runtime 的差异污染核心**：ClaudeCode 走 `@anthropic-ai/claude-agent-sdk` 的 `Query` 句柄；Native 走 HTTP SSE；Codex 走 `codex app-server` 子进程 + JSON-RPC。三者的中断语义、进程管理、僵死处理完全不同，现在这些差异直接混在流式主循环里，改一处易踩另一处。
- **Codex 进程管理的中断/僵死是独立难题**：`app-server-manager.ts` 里 binary 发现、版本选择、fatal config stderr 快失败、僵死 kill、orphan 进程规避——这些进程级复杂度不应污染"一次回合是什么"的核心逻辑。
- **中断后 thread/turn 状态残留**：Codex 的 thread/turn、ClaudeCode 的 `Query` 句柄（`conversation-registry.ts` 的 `lockId` 归属令牌）中断后若未关闭，下一轮发送语义错乱。

C2 把"一次回合的实时生命周期"抽成一个零框架核心：**phase 成为 StreamSession 实体的不变量**（只能经领域方法迁移，abort 保证翻终态）、**isStreaming gate 成为领域方法 `canAccept()`**（而非散落的 `phase==='active'` 比较）、**三个 Runtime 各一个适配器 + EventMapper**（把原生事件归一成 `AgentStreamEvent`，差异隔离在适配器内）。目标是让"CodePilot 高发的 stop/abort 卡死"在类型与不变量层面被切断，而不是靠每条分支的人工纪律。

## 3. 目标用户与价值

- **单机开发者用户**：在 SPA 里发消息、看流式输出、随时点"停止"——停止后 composer 立刻能再次发送，不会被卡在"仍在流式"的假象里；切 Runtime（ClaudeCode / Native / Codex）体验一致。
- **接入 C2 的其他上下文**：
...
- **拥有**：
  ...
  - 多 Runtime 抽象（`AgentRuntimePort` + 三个实现 `ClaudeSdkRuntimeAdapter` / `NativeRuntimeAdapter` / `CodexRuntimeAdapter`，各带 `EventMapper` 把原生事件归一）
- **不包含**：
...
| `ClaudeSdkRuntimeAdapter` | `claude-client.ts` + `conversation-registry.ts`（`Query` 句柄 + `lockId` 归属 + `abortConversation`） |
| `NativeRuntimeAdapter` | Native HTTP provider 的 SSE 流路径 |
| `CodexRuntimeAdapter` + 进程管理 | `codex/app-server-manager.ts`（binary 发现/版本选择/spawn/僵死 kill/orphan 规避）+ `codex/runtime.ts` 事件映射 |
...
- **S4 三 Runtime 差异隔离**：ClaudeCode / Native / Codex 各一个适配器 + EventMapper；核心的 `StreamSession` / 用例代码不出现 `@anthropic-ai/*`、`child_process`、`codex`、HTTP SSE 具体细节。Codex 的进程管理/中断/僵死锁在 `CodexRuntimeAdapter` 内。
...
- **风险（Codex 进程僵死）**：Codex app-server 可能 fatal config stderr 后 linger ~30s、可能 spawn EINVAL（Windows `.cmd` shim）、可能 stop 后进程僵死。这些必须锁在 `CodexRuntimeAdapter` 内部并 fail-fast，不让"一个 Runtime 的进程病"卡死整个 C2 的回合生命周期。
- **风险（中断后 turn/thread 残留）**：ClaudeCode 的 `Query` 句柄（`lockId` 归属）、Codex 的 thread/turn 中断后需显式关闭。C2 的 `abort()` 不变量必须包含"通知对应适配器关闭其 Runtime 侧 turn"，否则下一轮发送语义错乱（对齐 CLAUDE.md 优先排查方向第 4 点）。
