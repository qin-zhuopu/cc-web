...
| 当前 Runtime | 这轮是哪个 Runtime 在跑 | `StreamSession.runtimeKind`（`claude-sdk`/`native`/`codex`，发起时锁定） | 无（发起时必有） |
| Runtime 可用性 | Codex/ClaudeCode 能不能用 | `AgentRuntimePort.availability()`（Codex 探 binary/版本；不 spawn） | 探测失败 → `unavailable` + 原因，不显假 `ready` |
...
- FR-2.2 Runtime 选择基于 `providerId` 对应的 Provider 协议（经只读 `C7.ProviderRepository`）：ClaudeCode SDK / Native HTTP / Codex 三者之一，发起时锁定 `runtimeKind`。
...
- FR-3.3 优雅 interrupt 返回 Runtime 的权威状态（如 Codex/Native 的 runtime_status），若上游已确认终态则在微任务里 `reconcile` 把 phase 收敛到 terminal（不等 reader reject）；`running`/未知状态不纠正（force-abort 网仍兜底）。
- FR-3.4 abort 经 `SK.ErrorClassifier` 归为 `ABORTED` 独立错误类（非真实错误），phase 落 `terminal(aborted)`。
- FR-3.5 abort 必须通知对应 Runtime 适配器**关闭其 Runtime 侧 turn/thread/Query 句柄**（ClaudeCode 的 `Query` + `lockId` 归属、Codex 的 thread/turn），避免残留导致下一轮发送语义错乱。
...
- FR-4.2 每个 Runtime 适配器带一个 `EventMapper`，把该 Runtime 的原生事件（ClaudeCode SDK 消息 / Native SSE 帧 / Codex JSON-RPC 通知）归一成 `AgentStreamEvent`。核心只消费归一后的事件。
...
### FR-5 多 Runtime 抽象（`AgentRuntimePort` + 三适配器）
- FR-5.1 `AgentRuntimePort` 是 C2 定义的出站端口：`run(request): AsyncIterable<AgentStreamEvent>`（或等价的订阅式）、`interrupt(turnRef)`、`availability()`。三个适配器实现之。
- FR-5.2 `ClaudeSdkRuntimeAdapter`：封装 `@anthropic-ai/claude-agent-sdk` 的 `Query`，管理 `Query` 句柄注册 + `lockId` 归属（late-unregister no-op，对齐 `conversation-registry.ts`）、`abortConversation` + `Query.interrupt()` 组合中断。
- FR-5.3 `NativeRuntimeAdapter`：封装 Native HTTP provider 的 SSE 流，`AbortController` 中断。
- FR-5.4 `CodexRuntimeAdapter`：封装 `codex app-server` 子进程 + JSON-RPC，**内部隔离**：binary 发现/版本选择（多候选选最新）/spawn（Windows `.cmd` shim 经 cmd.exe）/fatal config stderr 快失败/僵死 SIGKILL/orphan 规避/thread-turn 中断。这些进程级复杂度**不得泄漏**到核心。
...
- NFR-1 **边界纯净**：`packages/core/agent-runtime/` 禁止 import `@anthropic-ai/*`、`better-sqlite3`、`@nestjs/*`、`node:child_process`、`codex` SDK；禁止直接 `Date.now()`/`randomUUID()`。SDK/进程/HTTP 细节全锁在适配器层。
...
- NFR-4 **Runtime 故障隔离**：任一 Runtime 的进程僵死/spawn 失败/init 失败必须 fail-fast 并归一成 `ClassifiedError`，不阻塞其他 Runtime、不卡死 C2 的回合生命周期。Codex 的 30s linger 类问题必须被 stderr 快失败提前切断。
- NFR-5 **可测**：三适配器与 EventMapper 可用录制的原生事件样本做表驱动归一测试；核心用例用假 `AgentRuntimePort` 测，无需真实 SDK/进程/网络（对齐 `CODEX_DISABLED=1` 思路）。
- NFR-6 **可观测**：回合关键路径（发起/中断/终态/Runtime 选择/进程 spawn）经 `SK.RuntimeLog` 记（脱敏后）source=`c2.stream`/`c2.runtime.codex` 等。
...
- AC-7（FR-4.1/4.2）三 Runtime 的录制原生事件样本经各自 EventMapper 归一成等价 `AgentStreamEvent` 序列（表驱动）；同一逻辑事件（如 tool_result 带 media）在三 Runtime 下归一结果结构一致。
...
- AC-10（FR-5.4）`CodexRuntimeAdapter` 隔离：核心包禁用 import 扫描对 `child_process`/`codex`/`@anthropic-ai/*` 0 命中；Codex fatal config stderr 触发 fail-fast（不等 30s）单测通过。
