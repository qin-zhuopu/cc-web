...
//
// 【本故事（c2-1-6）范围】只给端口签名与请求/引用形状，不实现具体适配器
//（属基础设施层，本期只有 ClaudeSdkRuntimeAdapter）、不实现 RuntimeRouter（属 epic-c2-2）、
// 不接 SDK / 子进程 / HTTP。
//
...
// AgentRuntimePort 是预留的未具名扩展点：核心不具名任何尚未实现的 agent。
 ...
 *  - handle：适配器内部不透明句柄（如 Query / AbortController / thread-turn 等），
 ...
 *
 * 实现位置：具体适配器（各带 EventMapper，位于基础设施层，本期只有 ClaudeSdkRuntimeAdapter），
 * 经 RuntimeRouter 按 runtimeKind 路由。AgentRuntimePort 是预留的未具名扩展点——
 * 将来新增 agent 时由其适配器实现本端口，核心不预先具名任何 agent。
...
export interface AgentRuntimePort {
  ...
  /** 非 spawn 的可用性探测（不发起真实回合；探测 binary/版本/配置等，不启进程）。 */
